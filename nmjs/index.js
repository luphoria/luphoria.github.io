...
while(counter <= counterGoal) {
    ...
    let geometry = new THREE.BoxGeometry(Math.random() * 20,Math.random() * 20,Math.random() * 20)
    let material = new THREE.MeshBasicMaterial({color: 0x0066FF})
    if(counterGoal / 4 > counter) material = new THREE.MeshBasicMaterial({color: 0x005CE6})
